import { Game, Position } from "../config/game.config.js";

const Y_cordinates : number[] = [0, 1, 2, 3, 4, 5, 6, 7]; // rank
const X_cordinates : number[] = [0, 1, 2, 3, 4, 5, 6, 7]; // file

export const swapKeyValue = (object: object) => {
    var ret = {};
...
Y_cordinates.forEach((Y: number) : void => {
    X_cordinates.forEach((X: number) : void => {
        let pieceInfo = {}

        if (Y <= 1) pieceInfo['teamColor'] = teamColors.white;
        else if(Y >= 6) pieceInfo['teamColor'] = teamColors.black;
...
export const stringToPosition = (position: string) : Position => {
    const x_y = position.split('o');

    try {
        if (x_y.length != 2) throw new Error("Function stringToPosition :: Position is invalid")
        else return {
            x: parseInt(x_y[0]),
            y: parseInt(x_y[1])
        }
...
export const checkEmptyTileOrNot = (position: Position, game: Game) : boolean => {
    return (
        game.board[`${position.x}o${position.y}`].teamColor === undefined &&
        game.board[`${position.x}o${position.y}`].pieceType === 0
    ) ? true : false;
}
...
export const positionExistOrNot = (position: string) => game.board[position] ? true : false;

export const findStreightPosition = ({
    position,
    direction,
    steps
}: {
...
export const findCrossPosition = ({
    position,
    forwardOrBack,
    leftOrRight,
    steps
...
export const findKnightPosition = ({
    position,
    forwardOrBack,
    leftOrRight,
    sideOrStreight
        ...
        if (sideOrStreight) {
            ...
            if (!leftOrRight) {
                ...
                if (leftCrossUp) {
                    ...
                    if (positionExistOrNot(PositionToString(leftOne))) return leftOne;
                    else return null;
                } else return null;
            } else {
                ...
                if (rightCrossUp) {
                    ...
                    if (positionExistOrNot(PositionToString(rightOne))) return rightOne;
                    else return null;
                } else return null;
            }
        }
...
export const findPosibleMove = (position: Position, game: Game) => {
    if (!isValidPosition(position)) throw new Error("Function findPosibleMove :: Invalid position provided.")
    else {
        var allPossibleMove = [];
        const strPosition = PositionToString(position);
        ...
        if (positionInfo.teamColor === 'undefined') throw new Error("Cant move empty tile");
        else {
            ...
            if (pieceType === 'pawn') {
                ...
                if (position.y === 1 && positionInfo.teamColor === teamColors.white) {
                    ...
                    if (isFirstEmpty) {
                        ...
                        if (isSecondEmpty) allPossibleMove.push(stringToPosition(`${position.x}o${position.y + 2}`));
                    }
                    ...
                    while(true) {
                        ...
                        if (foundCross) {
                            ...
                            if (isEmpty) {
                                ...
                                if (
                                    game.board[PositionToString(foundCross)].pieceType !== 0 &&
                ...
                Object.keys(knightDirectionsConfig).forEach(direction => {
                    ...
                    if (foundKnightMove) {
                        if (
                            checkEmptyTileOrNot(foundKnightMove, game)
                        ) {
                            allPossibleMove.push(foundKnightMove);
                        } else if (
                            game.board[PositionToString(foundKnightMove)].pieceType !== 0 &&
